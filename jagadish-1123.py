...
tr_dataset = tr_dataset.drop(
    ['Instance', 'Wears Glasses', 'Hair Color', 'Body Height [cm]'], axis=1)
# tr_dataset.info()
...
tr_dataset['AddnIncome'] = tr_dataset.AddnIncome.str.split(
    ' ').str[0].str.strip()
...
categorical_columns = [
    'Housing Situation', 'Satisfation with employer', 'Gender', 'Country',
    'Profession', 'University Degree'
]
tr_targetX, target_mapping, default_mapping = targetEncoding(
    tr_dataset, 'Total Yearly Income [EUR]', categorical_columns, 100, 10,
    0.05)
...
numerical_columns = [
    'Year of Record', 'Age', 'Work Experience in Current Job [years]'
]
...
ts_dataset = pd.read_csv('test data.csv',
                         na_values=null_values,
                         low_memory=False)
ts_dataset = ts_dataset.drop(
    ['Instance', 'Wears Glasses', 'Hair Color', 'Body Height [cm]'], axis=1)
#ts_dataset.info()

ts_dataset = ts_dataset.rename(columns=rename_columnsTo)

ts_dataset['AddnIncome'] = ts_dataset.AddnIncome.str.split(
    ' ').str[0].str.strip()
